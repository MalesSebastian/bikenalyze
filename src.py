...
hours = fullset.flatMap(lambda line: (line.split(",")[2].split(" ")[1],
                                      line.split(",")[5].split(" ")[1])).cache()
hours_pair = hours.map(lambda line: (line.split(":")[0], 1))
...
hours_for_weekend = date_and_hours_for_weekend.flatMap(
    lambda line: (
        line.split(",")[2].split(" ")[1],
        line.split(",")[5].split(" ")[1]))
hours_pair_weekend = hours_for_weekend.map(
...
hour_and_durations_sumsav = hour_and_duration.aggregateByKey(
                            (0, 0),
                            lambda a, b: (a[0] + int(b), a[1] + 1),
                            lambda a, b: (a[0] + b[0], a[1] + b[1]))
averages = hour_and_durations_sumsav.mapValues(lambda x: x[0] / x[1])
